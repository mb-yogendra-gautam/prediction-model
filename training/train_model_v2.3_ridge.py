"""
Model Training v2.3.0 - Ridge Regression
Daily Data with Multi-Horizon Predictions

Ridge Regression optimized for stability:
- alpha: 5.0 (L2 regularization)
- No feature selection (uses all ~79 features)
- Multi-output regression for 14 targets
"""
...
class RidgeTrainer:
    """Ridge Regression trainer for daily multi-horizon predictions"""
    ...
    def train_model(self, X_train, y_train):
        ...
        logger.info(f"Parameters: alpha=5.0")
    ...
    def cross_validate(self, X, y, groups, cv=5):
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
            ...
            ridge_fold = Ridge(alpha=5.0, random_state=42 + fold_idx)
            model_fold = MultiOutputRegressor(ridge_fold)
    ...
    def save_artifacts(self, version='2.3.0'):
        ...
        logger.info(f"Saving Ridge model v{version}...")

        output_dir = Path('data/models/v2.3')
        output_dir.mkdir(parents=True, exist_ok=True)

        joblib.dump(self.model, output_dir / 'ridge_model_v2.3.0.pkl')
        joblib.dump(self.scaler, output_dir / 'ridge_scaler_v2.3.0.pkl')
        joblib.dump(self.all_features, output_dir / 'ridge_features_v2.3.0.pkl')
        ...
        with open(output_dir / 'ridge_metadata_v2.3.0.json', 'w') as f:
...
def main():
    print("\n" + "="*80)
    print("MODEL TRAINING v2.3.0 - RIDGE REGRESSION (DAILY DATA)")
    print("="*80 + "\n")
    ...
    output_path = Path('reports/audit/model_results_v2.3.0_ridge.json')
