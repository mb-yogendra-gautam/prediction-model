"""
Model Training v2.3.0 - LightGBM
...
class LightGBMTrainer:
    """LightGBM trainer for daily multi-horizon predictions"""
    ...
    def load_and_prepare_data(self):
        ...
        logger.info(f"Train+Val: {len(X_train_val)}, Test: {len(X_test)}")
    ...
    def cross_validate(self, X, y, groups, cv=5):
        ...
        logger.info(f"Performing {cv}-fold CV...")
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups)):
            ...
            model_fold = MultiOutputRegressor(lgb_fold)
            ...
            logger.info(f"  Fold {fold_idx + 1}: R²={r2:.4f}, RMSE={rmse:.2f}")
        ...
        logger.info(f"CV: R²={np.mean(r2_scores):.4f} (+/- {np.std(r2_scores):.4f})")
    ...
    def evaluate_model(self, X_test, y_test):
        ...
        logger.info(f"Test: R²={overall_r2:.4f}, RMSE={overall_rmse:.2f}")
    ...
    def calculate_business_metrics(self, X_test, y_test):
        ...
        pct_errors = np.abs((y_pred[:, revenue_indices] - y_test[:, revenue_indices]) / (y_test[:, revenue_indices] + 1e-10)) * 100
    ...
    def save_artifacts(self):
        ...
        joblib.dump(self.model, output_dir / 'lightgbm_model_v2.3.0.pkl')
        joblib.dump(self.scaler, output_dir / 'lightgbm_scaler_v2.3.0.pkl')
        joblib.dump(self.all_features, output_dir / 'lightgbm_features_v2.3.0.pkl')
        ...
        with open(output_dir / 'lightgbm_metadata_v2.3.0.json', 'w') as f:
            json.dump(metadata, f, indent=2)


def main():
    print("\n" + "="*80)
    print("MODEL TRAINING v2.3.0 - LIGHTGBM")
    print("="*80 + "\n")
    ...
    with open('reports/audit/model_results_v2.3.0_lightgbm.json', 'w') as f:
        json.dump(results, f, indent=2)

    trainer.save_artifacts()

    print(f"\nTest R²: {test_results['overall_r2']:.4f}")
