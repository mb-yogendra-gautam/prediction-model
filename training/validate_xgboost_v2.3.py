"""
Comprehensive Validation for XGBoost Model v2.3.0
...
class XGBoostValidator:
    """Comprehensive validation for XGBoost v2.3.0"""
    ...
    def load_model(self):
        ...
        logger.info("Loading XGBoost v2.3.0 model...")

        model_dir = Path('data/models/v2.3')

        # Load model
        self.model = joblib.load(model_dir / 'xgboost_model_v2.3.0.pkl')
        self.scaler = joblib.load(model_dir / 'xgboost_scaler_v2.3.0.pkl')
        self.feature_names = joblib.load(model_dir / 'xgboost_features_v2.3.0.pkl')

        # Load metadata
        with open(model_dir / 'xgboost_metadata_v2.3.0.json', 'r') as f:
    ...
    def analyze_feature_importance(self) -> pd.DataFrame:
        ...
        # Get feature importance from first estimator (all have similar importances)
        ...
        logger.info(f"Top 5 features: {', '.join(importance_df.head(5)['Feature'].tolist())}")
    ...
    def plot_prediction_scatter(self, y_test, y_pred, save_dir='reports/figures'):
        ...
        plt.suptitle('XGBoost v2.3.0 - Prediction Accuracy (Test Set)', fontsize=16, fontweight='bold', y=0.995)
        plt.tight_layout()

        save_path = save_dir / 'xgboost_v2.3.0_predictions_scatter.png'
    ...
    def plot_residual_analysis(self, y_test, y_pred, save_dir='reports/figures'):
        ...
        plt.suptitle('XGBoost v2.3.0 - Residual Analysis (Test Set)', fontsize=16, fontweight='bold', y=0.995)
        plt.tight_layout()

        save_path = save_dir / 'xgboost_v2.3.0_residuals.png'
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved: {save_path}")
        plt.close()

    def plot_feature_importance(self, importance_df, save_dir='reports/figures'):
        ...
        bars = ax.barh(range(20), top_features['Importance'].values, color=colors,
        ...
        ax.set_title('XGBoost v2.3.0 - Top 20 Most Important Features', fontsize=14, fontweight='bold')
        ...
        save_path = save_dir / 'xgboost_v2.3.0_feature_importance.png'
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved: {save_path}")
        plt.close()
    ...
    def plot_horizon_performance(self, horizon_metrics, save_dir='reports/figures'):
        ...
        plt.suptitle('XGBoost v2.3.0 - Performance by Prediction Horizon', fontsize=16, fontweight='bold')
        plt.tight_layout()

        save_path = save_dir / 'xgboost_v2.3.0_horizon_performance.png'
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved: {save_path}")
        plt.close()
    ...
    def plot_error_distribution(self, y_test, y_pred, save_dir='reports/figures'):
        ...
        ax.set_title('XGBoost v2.3.0 - Error Distribution by Target', fontsize=14, fontweight='bold')
        ax.grid(True, alpha=0.3, axis='y')
        ...
        save_path = save_dir / 'xgboost_v2.3.0_error_distribution.png'
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        logger.info(f"Saved: {save_path}")
        plt.close()
    ...
    def get_model_size(self):
        ...
        model_path = Path('data/models/v2.3/xgboost_model_v2.3.0.pkl')
    ...
    def create_audit_report(self, validation_results, importance_df, save_dir='reports/audit'):
        ...
        audit_data = {
            ...
            'feature_importance_top_20': importance_df.head(20).to_dict('records'),
        ...
        # Save JSON report
        json_path = save_dir / 'xgboost_v2.3.0_validation_report.json'
        with open(json_path, 'w') as f:
            json.dump(audit_data, f, indent=2)
        logger.info(f"Saved JSON report: {json_path}")

        # Create human-readable text report
        txt_path = save_dir / 'xgboost_v2.3.0_validation_report.txt'
        with open(txt_path, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("XGBOOST MODEL v2.3.0 - COMPREHENSIVE VALIDATION REPORT\n")
            f.write("="*80 + "\n\n")

            f.write(f"Validation Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Model Type: XGBoost MultiOutputRegressor\n")
            ...
            for idx, row in importance_df.head(10).iterrows():
                f.write(f"{row['Rank']:>2}. {row['Feature']:<40} {row['Importance']:.6f}\n")
            f.write("\n")
    ...
    def verify_consistency_with_training(self):
        ...
        training_results_path = Path('reports/audit/model_results_v2.3.0_xgboost.json')
...
def main():
    print("\n" + "="*80)
    print("XGBOOST MODEL v2.3.0 - COMPREHENSIVE VALIDATION")
    print("="*80 + "\n")
    ...
    importance_df = validator.analyze_feature_importance()

    # Step 6: Measure inference performance
    print("\nStep 6: Measuring inference performance...")
    inference_performance = validator.measure_inference_performance(X_test)

    # Step 7: Get model size
    ...
    validator.plot_feature_importance(importance_df)
    ...
    # Step 9: Create audit report
    print("\nStep 9: Creating audit reports...")
    validation_results = {
    ...
    audit_data = validator.create_audit_report(validation_results, importance_df)

    # Step 10: Verify consistency
    print("\nStep 10: Verifying consistency with training results...")
    print("-" * 80)
    ...
    print("\nReports saved:")
    print("  - reports/audit/xgboost_v2.3.0_validation_report.json")
    print("  - reports/audit/xgboost_v2.3.0_validation_report.txt")
    print("\nVisualizations saved:")
    print("  - reports/figures/xgboost_v2.3.0_predictions_scatter.png")
    print("  - reports/figures/xgboost_v2.3.0_residuals.png")
    print("  - reports/figures/xgboost_v2.3.0_feature_importance.png")
    print("  - reports/figures/xgboost_v2.3.0_horizon_performance.png")
    print("  - reports/figures/xgboost_v2.3.0_error_distribution.png")
    print("\n" + "="*80 + "\n")
